 ...
 * date   2015 05 19     
 */
class fenchenglistAction extends backendAction
{
    ...
    public function index(){
        $list=$this->_mod->where(array('state'=>1))->order('id desc')->select();
    ...
    public function search(){
        if ($_GET['orderId']) {
            ...
            $list=$this->_mod->where($data)->select();
        }
        if ($_GET['userName']) {
            $data1['state']=1;
            $data1['username']=array('like','%'.$_GET['userName'].'%');
            $user=M('user')->where($data1)->field('id')->find();
            $data=array();
            $data['state']=1;
            //$data['user_id|uid']=$user['id'];
			$data['uid']=$user['id'];//2016-04-29 by shuguang 修改 只按照分成所属筛选
            $list=$this->_mod->where($data)->select();
            $data['userName']=$_GET['userName'];
        }
        if ($_GET['time_start'] && $_GET['time_end']) {
            $data['add_time']= array('between', array(strtotime($_GET['time_start']),strtotime($_GET['time_end'])));
            $data['state']=1;
            $list=$this->_mod->where($data)->select();
	...
	public function excelout(){
        ...
        */
        ...
        $filename="分成明细";
        $headArr=array("ID","提成率","订单号","分成金额","订单金额","分成所属","下单所属","更新时间");
