 ...
 */
		...
		if ($res !== false) {//审核通过
			...
			if($check['hq_status'] == 0){//总部审核之前的状态为未处理
				...
				if($user['hq_status'] == 1){//上级审核通过了
					//将发送的消息写进后台
					$message = D('messagelist');
					$message->user_id = $user['userid'];
					$message->recom = $user['userid'];
					$message->pid = $user['id'];
					$message->type = 2;
					$message->order_id = 0;
					$message->time = time();
					$message->status = 0;
					$message->content = "
					恭喜{$user['username']}，您的申请已经审核成功！欢迎加入我们团洋范大平台，我们将送您到微商的下一个风口！名片如下：<br/>
					姓名：{$user['username']}<br/>
					微信号：{$user['wxname']}<br/>
					手机号码：{$user['phone_mob']}<br/>
					您可以通过菜单【卖家】-【邀请链接】获取您的邀请链接(用于发展代理商).卖家中心首次登陆密码:123456,为了账号安全,请登录后修改。
					";
					if($message->add()){
						$message->commit();
						$data = array(
						...
						$message->status = 0;
						$message->content = "
							尊敬的{$share['username']}{$share['name']}，有一名{$lastlv['name']}归到您的麾下，名片如下：<br/>
							姓名：{$user['username']}<br/>
							微信号：{$user['wxname']}<br/>
							手机号码：{$user['phone_mob']}<br/>
							如有疑问请在公众号内咨询在线客服！
					...
					$message->status = 0;
					$message->content = "
						尊敬的{$share['username']}，很遗憾，您的代理{$user['username']}，未能通过后台审核，如有疑问请在公众号内咨询客服！
					";
					if($message->add()){
						$message->commit();
						...
						$message->status = 0;
						$message->content = "
							尊敬的{$user['username']}您好，您的代理申请未能审核通过！如有疑问请在公众号内咨询客服！
						";
						$message->add();
						exit;
					}else{
				...
				$data1['uid']		 = $share['uid']+1;//shares是直接上级
				$data1['ren_time']	 = time();
				...
				$message = D('messagelist');
				$message->user_id =$user['userid'];
				$message->recom = $reshares['id'];
				$message->pid = $this->_get('id','intval');
				$message->type = 2;
				$message->order_id = 0;
				$message->time = time();
				$message->status = 0;
				$message->content = "
					尊敬的{$reshares['username']}您好！您有一笔新的收入提醒：<br/>
					收入类型：代理培训奖金<br/>
					收入金额：{$data['fencheng']}<br/>
	...
	public function get_fc($id,$fencheng,$mid,$n,$m){
		...
		$message->status = 0;
		$message->content = "
			尊敬的{$share['username']}您好！您有一笔新的收入提醒：<br/>
			收入类型：代理培训奖金<br/>
			收入金额：{$data['fencheng']}<br/>
			到账时间:".date("Y-m-d H:i:s")."<br/>
			如有疑问请在公众号内咨询在线客服！
