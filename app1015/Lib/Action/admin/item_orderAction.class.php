...
class item_orderAction extends backendAction {
    public function _initialize() {
        ...
        $order_status=array(1=>'待付款',2=>'待发货',3=>'待收货',4=>'完成',6=>'申请退款中',7=>'申请退款失败',8=>'申请退款成功');
...
*/
	...
	public function update_order(){
		...
		$where1['a.status']	= 3;
     	...
     	foreach ($orders as $key1 => $item_orders) {
     		$data['status']	   = 4;//收到货
	     	$where['orderId']  = $item_orders['orderId'];
	     	$where['status']   = 3;
	     	$wxsend            = new Wxsend();    
		    ...
		    {	
		     	...
		     	$order_details = $order_detail
		     					->alias("a")
		     					 ->join("__ITEM__ as b on a.itemId = b.id ")
		     					 ->where("a.orderId = '".$item_orders['orderId']."'")
		     					 ->field('a.*,b.fencheng')
		     					 ->select();
		    	foreach ($order_details as $k => $val)
		     	{
					...
					//卖咖店铺分成
					$fencheng['price']	 = $val['price'];
					$fencheng['user_id'] = $item_orders['userId'];
					$fencheng['add_time']= time();
					$fencheng['state'] 	 = 1;
					$fencheng['orderId'] = $val['orderId'];
					$fencheng['royalty'] = $val['fencheng'];
					$fencheng['fencheng']= round($val['profit']*$val['fencheng'],2);
					$fencheng['uid']   	 = $val['shopid'];
					$fencheng['fencheng']= $val['profit']*$val['fencheng'];
					$res = M("user_fengchengllist")->add($fencheng);
					$wxsend->SR($item_orders['wechatid'],$fencheng['fencheng'],date("Y-m-d H:i:s"));
					
					if ($item_orders['uid'] == 3) {
						foreach ($fclvArr as $key => $value){
							if($key == 0) {
								if (!empty($item_orders['recom'])) {
									$user = M('user')->where(array('id'=>$item_orders['recom']))->field('wechatid')->find();
									$fencheng['royalty'] = $value;
									$fencheng['fencheng']= round($val['profit']*$val['fencheng']*$value,2);
									$fencheng['uid']   	 = $item_orders['recom'];
									$res = M("user_fengchengllist")->add($fencheng);
									$wxsend->SR($user['wechatid'],$fencheng['fencheng'],date("Y-m-d H:i:s"));
								}
							}else{
								$share = M('user')->where(array('id'=>$shares['id']))->field('id,username,shares,status,wechatid')->find();
								if ($share['status'] == 1) {
									$fencheng['royalty'] = $value;
									$fencheng['fencheng']= round($val['profit']*$val['fencheng']*$value,2);
									$fencheng['uid']   	 = $share['id'];
									$res = M("user_fengchengllist")->add($fencheng);
									$wxsend->SR($share['wechatid'],$fencheng['fencheng'],date("Y-m-d H:i:s"));
								}
								$shares = M('user')->where(array('id'=>$share['shares']))->field('id,shares,status')->find();
							}
						}
					}else if($item_orders['uid'] == 2){
						foreach ($fclvArr as $key => $value) {
							if ($key == 2) {
								//获取对应用户组分成率
								$share = M('user')->where(array('id'=>$shares['id']))->field('id,shares,status,wechatid')->find();
								if ($share['status'] == 1) {
									$fencheng['royalty'] = $value;
									$fencheng['fencheng']= round($val['profit']*$val['fencheng']*$value,2);
									$fencheng['uid']   	 = $share['id'];
    ...
    public function fahuo()
    {
        ...
        if (IS_POST&&$this->_post('orderId','trim')) {
            ...
            $date['status']=3;
            if($mod->where(array('orderId'=>$orderId))->data($date)->save() !== false){
				$orderlist = M('item_order')
				...
				$message->user_id = $orderlist['userId'];
				$message->recom = $orderlist['userId'];
				$message->pid = 0;
				$message->type = 1;
				$message->order_id = $orderlist["orderId"];
				$message->time = time();
				$message->status = 0;
				$message->content = "
					尊敬的{$orderlist['username']}您好！您购买的商品订单号为:{$orderlist["orderId"]}的宝贝已发货，详细信息如下：<br/>
					快递:{$orderlist["userfree"]}<br/>
					快递单号:{$orderlist["freecode"]}<br/>
					发货时间:".date('Y-m-d H:i:s',$orderlist["fahuo_time"])."<br/>
					如有疑问请在公众号内咨询客服！
				";
				if($message->add()){
					$message->commit();
					...
					$content = "亲爱的饭团{$orderlist['username']}，感谢您对团洋范的支持！您订购的宝贝已经乘坐{$orderlist["userfree"]}({$orderlist["freecode"]})迫不及待的奔向您啦，请注意签收。祝您生活愉快！";
	...
	public function excelout(){
        ...
        $data=$this->_mod
			   ->where($map)
			   ->join("left join __USER__ as b on weixin_item_order.shopid = b.id left join weixin_itembase c ON weixin_item_order.baseid=c.id")
			   ->field('weixin_item_order.id,weixin_item_order.orderId,weixin_item_order.order_sumPrice,weixin_item_order.address,b.merchant,weixin_item_order.address_name,weixin_item_order.mobile,weixin_item_order.add_time,weixin_item_order.freetype,c.basename')
			   ->select();  
        ...
        foreach ($data as $k => $v) {
            ...
            $print_data[$k]['order_sumPrice'] = $v['order_sumPrice'];//订单金额
		...
		//print_r($data);exit;	 
        $filename="订单详情";
		$headArr=array("ID","订单编号","发货地点","快递公司","快递单号","发货时间","收货地址","收货人","联系方式","下单时间","身份证姓名","身份证号码","订单详情","单价","数量","快递费","应付总金额","订单金额","所属分享店铺","快递备注","免税仓");
