<?php 
		...
		class SongjuanAction extends backendAction {
			...
			*/
			public function index(){
				...
				if($_GET['keyword']){
					...
					//调用分页
					$list = $dj->order('id desc')->where(array('type'=>1))->select();
				}
				$count =count($list);
			...
			*/
			...
			public function addtemp(){
				...
				if($_POST){
					...
					//插入数据
					if($diajin->field('djname,djmoney,djcondition,start,end')->data($data)->add()){
						
						$this->success('代金券生成成功',U('Songjuan/index'));
			...
			public function del(){
				...
				if($daijin->where('id='.$id)->delete()){
						
					$this->success('删除成功', U('Songjuan/index'));
			...
			public function fdjadd(){
				...
				if($_POST){
					...
					$res=$daijin->add($data);
					if($res!==false){
						$this->success('添加成功',U('Songjuan/index'));
					}else{
						$this->error('添加失败');
					}
					...
					for( $i = 0; $i< $count ;$i++ )
					{
						...
						if($diajin->field('djuser,djusername,djname,djcode,djmoney,djcondition,djstart,djend')->data($data)->add()){
							...
							$this->success('代金券发放成功',U('Songjuan/index'));
			...
			public function updatedj(){
				...
				if($daijin->where('id='.$data['id'])->filter('strip_tags')->save($data)!==false){
					...
					$this->success('代金券更新成功',U('Songjuan/index'));
				...
				if($_POST){
					...
					for( $i = 0; $i< $num ;$i++ ){
						...
						if($diajin->field('djname,djcode,djmoney,djcondition,djstart,djend')->data($data)->add()){
							...
							$this->success('代金券批量生成成功',U('Songjuan/index'));
