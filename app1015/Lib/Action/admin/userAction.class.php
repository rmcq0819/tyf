...
class userAction extends backendAction
{
	...
	public function zg_update(){
		  ...
		  $user_data['up_user'] = $user_form['shares']; //上级id
    ...
    public function edit()
    {
        ...
        if (IS_POST) {
            ...
            if (false !== $mod->save($data)) {
                ...
                $this->add_log('修改用户资料');
				//手动为用户添加提成记录
				$add_cny = $this->_post('add_cny'); //提成金额
				$add_cont = $this->_post('add_cont'); //添加提成理由
				$user_source = $this->_post('user_source'); 
				$add_points = $this->_post('add_points'); 
