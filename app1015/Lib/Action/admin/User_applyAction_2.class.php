 ...
 */
		...
		if ($res !== false) {//审核通过
			...
			if($check['hq_status'] == 0){//总部审核之前的状态为未处理
				...
				/*dump($user['wechatid'].' '.$user['username'].' '.$user['wxname'].' '.$user['phone_mob'].' '.$user['hq_status'].' '.$date);*/
				...
				/* if (!empty($user['recom'])) {    //直接一级 +200
					$data['fencheng'] = $fc[1];   //200
					$data['price'] = $fc[1];
					$data['uid'] = $user['recom'];
					$data['add_time'] = time();
					$data['user_id'] = $check['userid'];
					$reshares = M('user')->where(array('id'=>$user['recom']))->field('wechatid')->find();
				...
				$data['add_time'] = time();
				$data['user_id'] = $check['userid'];
				$reshares = M('user')->where(array('id'=>$tree[$num-2]))->field('wechatid')->find();
				$wxsend->SR($reshares['wechatid'],$data['fencheng'],date("Y-m-d H:i:s"),"代理培训奖金");
				M("user_fengchengllist")->add($data);
				
				
				$this->get_fc($tree[$num-2],$fencheng['royalty'],$check['userid'],2,3);//控制为四级   23629
			}
	...
	public function get_fc($id,$fencheng,$mid,$n,$m){
		...
		$user = M('user')->where(array('id'=>$id))->field('id,uid,wechatid,shares,shares_tree')->find();
		$tree = explode('|',$user['shares_tree']);//直接上级的shares_tree
		$m--;
		if($m<0||count($tree)<3){
			return 0;
		...
		if ($user['uid'] >1&&$user['uid'] <5) {
			$share = M('user')->where(array('id'=>$tree[$num-2]))->find();
			$data['type'] = 1;
			$data['fencheng'] = $fc[$n];
			$data['price'] = $fc[$n];
			$data['uid'] = $share['id'];   // 78
			$data['add_time'] = time();
			$data['user_id']  = $mid;
			$ret=M("user_fengchengllist")->add($data);
			//$wxsend->SR($share['wechatid'],$data['fencheng'],date("Y-m-d H:i:s"),"代理培训奖金");
			/*dump($share['wechatid'].' '.$data['fencheng'].' '.date("Y-m-d H:i:s")."代理培训奖金");*/
		
			$this->get_fc($share['id'], $fencheng,$mid,0,$m);
		}
