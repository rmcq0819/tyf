   ...
   public function getmenu()
   {
   	$keyword = array();
    	...
    	foreach ($topmemu as $key )
    	{
    		...
    		if(count($nextmenu)!=0)//有下级栏目
    		{
                 foreach ($nextmenu as $key2)
                 {
                 	if($key2['key']){
						$kk[]=array('type'=>'view','name'=>$key2['name'],'url'=>$key2['key']);
					}else{
    		...
    		{
                if($key['key']){
					$keyword['button'][]=array('type'=>'view','name'=>$key['name'],'url'=>$key['key']);
				}else{
    ...
    public function creat_custom_menu()
    {
    	...
    	if($ACCESS_LIST['access_token']!='')
    	{
         ...
         $msg=$this->curl_menu($access_token,preg_replace("#\\\u([0-9a-f]+)#ie", "iconv('UCS-2BE', 'UTF-8', pack('H4', '\\1'))", $data));
        if($msg['errmsg']=='ok')
        {
        	$this->success('创建自定义菜单成功!');
        }
        else {
        	$this->error('创建自定义菜单失败!'.json_encode($msg));
        }
