...
*/
    ...
    public function excelout(){
        ...
        $data = M('order_detail')->join("a left join weixin_item_order b on a.orderid=b.orderId")->field("b.id,a.title,a.orderId,a.quantity,a.price,a.quantity*a.price as order_sumPrice,b.add_time")->where($data)->order('a.id asc')->select();
        /*
        $data=M('item_order')
			   ->where($data)
       ...
       /* dump($re[0]['data']);
        ...
        $filename = "销售明细";
        $headArr = array("ID","订单号","下单时间","商品名称","数量","售价","总价");
        $printdata = array();
