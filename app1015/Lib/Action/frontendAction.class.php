...
frontendAction extends baseAction {
    ...
    public function _initialize() {
       ...
       /*  if(!$this->is_weixin()){
		...
		$shop_id = $this->_get('shares');
		$user_id = $this->visitor->info['id'];
		$user_shop = M('seller')->where(array('userid'=>$user_id,'shopid'=>$shop_id))->select();
		if(!$user_shop && $shop_id!=""){
			$data = D('seller');
			$data->userid = $user_id;
			$data->shopid = $shop_id;
			$data->addtime = time();
...
/***********************************************************app接口-start****************************************************************/	
	...
	public function getInfo(){
		...
		if(!$openid){
			...
			$user_info = M('user')->where(array('wechatid'=>$openid))->find();
			if($user_info['uid']<4){
