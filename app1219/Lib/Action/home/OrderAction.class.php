...
class OrderAction extends userbaseAction {
	...
	public function confirmOrder()//确认收货
	{
        ...
        {
			/* //更新detail表中的状态为已完成         这样处理未考虑订单中有退款之类的订单条目
			M('order_detail')->where(array('orderId'=>$orderId))->save(array('status'=>4)); */
			
			
            //获取订单信息和商品信息       只需考虑能正常收货的商品即可
            $order_detail = M('order_detail')
                ->where(array('orderId'=>$orderId,'status'=>3))
                ->select();

            foreach ($order_detail as $k => $val) {
                $profitPrice += $val['profit']; //获取订单利润
                $orderPrice += $val['price']*$val['quantity']; //获取订单单价
                // 减掉对应商品的库存数量
                $item->where('id='.$val['itemId'])->setInc('buy_num',$val['quantity']);
                $shares['id']= $item_orders['shares'];
				
				//更新detail表中的状态为已完成        更新正常收货了的
				M('order_detail')->where(array('id'=>$val['id']))->save(array('status'=>4));
            }
            ...
            foreach($royArr as $rk=>$rv){
                ...
                $wxsend->SR('oOejpwiK88gEkMvMHJUxe5JhN6lE',round($profitPrice*$rv,2).' '.$shop_shares['wechatid'][$rk],$time); //测试样例
            }
            //dump($fcdata);die;
