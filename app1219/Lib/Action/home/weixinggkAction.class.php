<?php
class weixinggkAction extends userbaseAction
{
	public $u_phone 	= '';
	public $u_name 		= '';
    ...
    private function getResult($priearr){
		...
		if(($zp_con_info['hd_cishu'] - $you_num) > 0)//如果有抽奖次数
		{
			...
			//添加到优惠券表中
			$zplistjifen = M('ggk_list');
			$zp_list_infojifen	= $zplistjifen->where("id=$listid")->find();
			
			$userjifen	= M('item_jifen');
			$youhuiquan = $zp_list_infojifen['praisecontent'];
			
			$jifen['uid']		= $user_id;
			$jifen['title']	= $youhuiquan;
			$jifen['jifen']		= 0;
			$jifen['add_time']	= time();
			$jifen['state']		= 0;
			...
			if($listid != 7)//如果中奖
			{
				$msg = "恭喜你获得【".$zp_list_info['praisecontent'].'】';
			}else{
    ...
    private function getRand($proArr,$proCount){	
    	...
    	foreach($proCount as $key=>$val){
    		if($val==0){
    			...
    			$proSum=$proSum+floor($proArr[$key]*$sail10Tmp);
    		}
    	}
    	//概率数组循环
    	foreach ($proArr as $key => $proCur) {
    		if($proCount[$key]==0){
        		...
        		if ($randNum <= floor($proCur*$sail10Tmp)) {
        			$result = $key;
           	 		break;
        		}
    		}
