...
class UserAction extends userbaseAction {
    ...
    */
	...
	public function sales_list(){
        ...
        ->field("a.*,b.price,b.id as odId,b.quantity")
        ->where($where)
        ->select();
          ...
          $shares= $order['shares'];
            ...
            foreach($royArr as $rk=>$rv){
				...
				if($order['is_sendprice'] == 0){
					...
					$wxsend->SR('oOejpwiK88gEkMvMHJUxe5JhN6lE',round($profitPrice*$rv,2)."(待结算)".' '.$shop_shares['wechatid'][$rk],$time); //测试样例
				}
            }
