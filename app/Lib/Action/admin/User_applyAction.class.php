 ...
 */
		...
		if ($res !== false) {//审核通过
			...
			if($check['hq_status'] == 0){//总部审核之前的状态为未处理
				...
				if($user['hq_status'] == 1){
					//总部审核通过后为用户下发总额度为1000元的优惠券组合
					$uc_mod = M('user_coupon');
					$couTpl = M('coupon_templet')->where(array('id'=>array('in','11,12,13,14')))->select();
					$time = time();
					$etime = $time+30*24*3600;
					foreach($couTpl as $key=>$val){
						$cd['status'] = 0;
						$cd['userId'] = $user['userid'];
						$cd['couponId'] = $val['id'];
						$cd['add_time'] = $time;
						$cd['end_time'] = $etime;
						$cd['is_prize'] = '1';//is_prize为1表示成为店长后系统赠送的优惠券
						for($i=0;$i<$val['counts'];$i++){
							$uc_mod->add($cd);
						}
					}
					$data = array(
	...
	public function excelout(){
        ...
        if($_POST['add_time_start'] && $_POST['add_time_end']){
            $map['add_time'] = array('between', array(strtotime($_POST['add_time_start']),strtotime($_POST['add_time_end'])));
        }
        if($_POST['part']){
            $map['part'] = array('eq',$_POST['part']);
        }
        ...
        foreach ($data as $key => $value) {
            ...
            $print_data[$key]['part'] = $value['part'];
         	$print_data[$key]['address'] = $value['address'];

            if($value['shares']!=0){
                $value['shares'] = M('user')->where(array('id'=>$value['shares']))->getField('username');
            }else{
                $value['shares'] = $value['shares_name'];
            }
            $print_data[$key]['shares'] = $value['shares'];

            if($value['recom']!=0){
                $value['recom'] = M('user')->where(array('id'=>$value['recom']))->getField('username');
            }else{
                $value['recom'] = $value['shares_name'];
            }

         	$print_data[$key]['recom'] = $value['recom'];
            ...
            $value['add_time'] =  date('Y-m-d H:i:s',$value['add_time']);
         	$print_data[$key]['add_time'] = $value['add_time'];
        ...
        $headArr=array("ID","用户名","微信号","联系方式","申请类型","联系地址","团队上级","直接推荐人","支付时间","申请时间","上级审核时间","上级审核状态","总部审核状态");
