 ...
 */
	...
	public function update_orderid(){
		$fping = M('item_order')->where(array('userId'=>$this->visitor->info['id'],'status'=>1,'fping'=>1))->select();
		if(count($fping) > 0){
			$dingdanhao = date("Y-m-dH-i-s");
			$dingdanhao = str_replace("-","",$dingdanhao);
			$dingdanhao .= rand(1000,2000);
			foreach($fping as $key => $val){
				$order_type = strstr($val['orderId'],'-'); //订单类型
				$new_orderId = $dingdanhao.$order_type;
				$detil_id = substr($val['orderId'],0,18);
				$detil_info = M('order_detail')->where("orderId LIKE '$detil_id%'")->select();
				M('item_order')->where(array('id'=>$val['id']))->save(array('orderId'=>$new_orderId));
			}
			foreach($detil_info as $k => $v){
				$order_type = strstr($v['orderId'],'-'); //订单类型
				$new_orderId = $dingdanhao.$order_type;
				M('order_detail')->where(array('id'=>$v['id']))->save(array('orderId'=>$new_orderId));
			}
		}
	}
    /**
