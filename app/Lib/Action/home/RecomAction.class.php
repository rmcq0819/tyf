...
class RecomAction extends frontendAction {
	...
	public function index(){
		...
		$this->assign('share_data',$share_auth);
       	...
       	$this->assign('u_level',$this->visitor->info['uid']);
        //找出全部的满99的商家
		...
		//对该代理商做特殊处理
		if($_SESSION['shopid'] == 2965 && !is_array($satisfy)){
			$higher[0]['id'] = 2965;
			$higher[0]['username'] = '林艺丹';
			$higher[0]['shares_tree'] = '-|78|-';
		}
		...
		if (IS_AJAX) {
			...
			if($apply['part']==1 && !empty($apply)){
				...
				// 判断分享者等级
                if($data['juese']==2){  //经纪人
                    if(!is_array($satisfy) && $_SESSION['shopid'] != 2965){
				...
				if($this->visitor->info['uid'] == 5){
					$data['shares_tree'] = $this->visitor->info['shares_tree'];
				}else{
				...
				if(empty($_SESSION['is_submit'])){
					...
					$ret = array('title'=>'请勿重复点击申请按钮！','msg'=>'申请已成功提交','code'=>1,'url'=>'index.php?m=User&a=index');
					echo json_encode($ret);
					exit;
				}
				 if ($res !== false) {
