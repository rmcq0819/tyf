...
class OrderAction extends userbaseAction {
	...
	public function orderTK() {
		if (IS_POST) {
			...
			$ck = $_POST['ck'];
			if(M('item_ordertk')->where(array('orderId'=>$orderId,'detailId'=>$detail_id))->find()){
				$this->error('您已申请过退款无需重复申请，请耐心等待处理结果！');
			}
			if(empty($yuanyin)){
				$this->error('退款原因还未填写');
			}
			...
			$ioMd = M("item_order");
			if(!empty($detail_id)){
				$res=$odMd->where(array("id"=>$detail_id))->save(array("status"=>$to_status));
			}else{
				$res=$ioMd->where(array("orderId"=>$orderId))->save(array("status"=>$to_status,'tuihuo_time'=>time()));
				$odMd->where(array("orderId"=>$orderId))->save(array("status"=>$to_status));
			}
	...
	public function jiesuan(){//结算
		...
		if(count($_SESSION['cart'])>0)
		{
			...
			$card_info = $this->_get('card_info','trim');
			$this->assign('address_list', $address_list);
			$this->assign('addr_id',$addr_id);
			$this->assign('cart_count',count($_SESSION['cart']));
...
/*=======================by lyye 2014-04-08=======================*/
	...
	{
		if(IS_POST && count($_SESSION['cart']) > 0 && $_POST['cart_count'] > 0)//1.订单未生成情况下  生成订单号、分单   
		{
		   import('Think.ORG.Cart');// 导入分页类
           $cart=new Cart();	
		   $user_address=M('user_address');
		   ...
		   if($this->visitor->info['username']){
			...
			if($data['goods_sumPrice']<99){
				...
				if($isfic['is_fictitious'] == 1 || $store['is_store']==1 ||$_SESSION['count']>=3){
					$data['order_sumPrice'] = $data['order_sumPrice']-60;	//应付款的
				}else{
				...
				foreach ($itemlist as $key=>$item)
				{
					...
					if($item['itemtype']==0){
						...
						foreach($item['goods'] AS $key1=>$item1){
							...
							$goods_size = $item_goods->where(array('id'=>$item1['id']))->field('size')->find();
							//获取该商品的规格
							$size=explode('|',$goods_size['size']);
		...
		$coupon = M('user_coupon')->join('a left join weixin_coupon_templet b on a.couponId=b.id')
								  ->where('a.status=0 and b.condition <= '.$orderinfo[0]['goods_sumPrice'].'  and b.kind =1 and userId='.$this->visitor->info['id'].' and a.end_time >= '.time())
								  ->field('b.*,a.id as ucId,a.end_time as etime')
								  ->select();
		$this->assign('coupon',$coupon);
		$this->assign('count',count($coupon));
	...
	public function paysuccess(){
		...
		if(M('order_detail')->where(array('itemId'=>array('in','5221,5222,5223'),'orderId'=>array('like',$orderid.'%')))->find()){
			$is_give = $order_mod->where($where)->getField('is_give');
			if($is_give == 0){
				$ct_mod = M('coupon_templet');
				$uc_mod = M('user_coupon');
				$user_mod = M('user');
				...
				$order_mod->where($where)->save(array('is_give'=>1));
				$this->assign('give_redbag',1);
			}
		}
		if(M('order_detail')->where(array('itemId'=>array('in','5313,5314,5315'),'orderId'=>array('like',$orderid.'%')))->find()){
			$this->assign('lb','1');
		}
	...
	public  function end()
	{
		if(IS_POST)
		{
			...
			if($point != 0 && $point >= 100){
				$orderext = M('messagelist')->where("order_id LIKE '$dingdanhao%' and type=7")->find();
				if(!is_array($orderext)){
					$datas['user_id'] =$order['userId']; //用户id
					$datas['recom'] = $order['userId']; //用户id
					$datas['type'] = 7; //范票抵扣
					$datas['order_id'] = $order['orderId']; //订单id
					$datas['time'] = time();
					$datas['status'] = 0; // 默认未读状态
					$datas['points'] = $point;
					$datas['lastpoint'] = M('user')->where(array('id'=>$order['userId']))->getField('points');
					if(M('messagelist')->add($datas)){
						$order_data['order_sumPrice'] = $order['order_sumPrice'] - $point/100;
						$order_data['cash_price'] = $point;
						$order_data['buck_time'] = time(); //抵扣时间
						$order_data['fping'] = 1; //抵扣标识
						M('item_order')->where("orderId LIKE '$dingdanhao%'")->save($order_data);
						M('user')->where(array('id'=>$order['userId']))->setDec('points',$point);
					}
				}
			}
			$disPrice = 0;
...
//物流查询 
	public function logistics(){
		...
		$detail_id 	 =$_GET['detail_id'];
		$order = M('order_detail')->join("a left join weixin_item_order as b on a.orderId = b.orderId")
								  ->field('a.*,b.address_name,b.mobile,b.address,b.order_sumPrice,b.yunfei,b.add_time,b.qg_time')
		...
		$tuihuo_time = M('item_ordertk')->where(array('datailId'=>$detail_id,'orderId'=>$order['orderId']))->getField('add_time');
		$start_time = $order['add_time']+220; //开始处理订单时间
		$dist_time = $order['add_time']+454; //开始配货时间
		$this->assign('start_time',$start_time);
		$this->assign('dist_time',$dist_time);
		$this->assign('tuihuo_time',$tuihuo_time);
		$this->assign('act_time',time());//获取当前时间
		$this->assign('refund_time',time()-30*24*3600);//获取当前时间
	    $this->display();
	}
