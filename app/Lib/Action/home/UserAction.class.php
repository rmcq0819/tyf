    ...
    */
	...
	public function addaddress(){
        if(IS_POST){
			...
			$is_redirect = $this->_post('is_redirect','trim');
     ...
     */
     public function index(){
		...
		if(time() > $end_buck['buck_time']+100 && $end_buck['buck_time']!=0){
			$msgdata['user_id'] = $end_buck['shopid']; 
			$msgdata['recom'] = $this->visitor->info['id']; 
			$msgdata['type'] = 22; 
			$msgdata['order_id'] = $end_buck['orderId']; //订单id
			$msgdata['time'] = time();
			$msgdata['status'] = 0; // 默认未读状态
			$msgdata['points'] = $end_buck['cash_price'];
			if(M('messagelist')->add($msgdata)){
		...
		if($uid == 4){
			...
			//针对该代理商做特殊处理
			if($_SESSION['shopid'] == 2965){
				$this->assign('satisfy',100);
			}else{
		...
		if(time() > $robredbag['robtime']+3600*1 && count($robredbag) > 0){
            $rdata['user_id'] = $this->visitor->info['id']; //用户id
            $rdata['recom'] = $this->visitor->info['id']; //用户id
            $rdata['pid'] = $robredbag['Id'];
            $rdata['type'] = 11; //系统退还
            $rdata['order_id'] =0; //订单id
            $rdata['time'] = time();
            $rdata['status'] = 0; // 默认未读状态
            $rdata['points'] = $robredbag['content'];
            M('messagelist')->add($rdata);
        ...
        if ($_GET['status'] == 2 && !empty($_GET['orderid'])){
			//支付成功后更改订单详情状态信息
			M('order_detail')->where(array('orderId'=>array('like',$_GET['orderid'].'%')))->save(array('status'=>2));
            //获取订单和店铺代理信息
            $order        = M('item_order')
          ...
          $shares= $order['shares'];
			...
			if($order){
				...
				if($order['userId'] == $order['shopid']){
					$givedata['user_id'] =$order['userId']; //用户id
					$givedata['recom'] = $order['shopid']; //商家id
					$givedata['type'] = 5; //购物积分
					$givedata['order_id'] = $order['orderId']; //订单id
					$givedata['time'] = time();
					$givedata['status'] = 0; // 默认未读状态
					$givedata['points'] = $points;
					if(M('messagelist')->add($givedata)){
			...
			//以上分支走完后，如用户未收到待结算提醒，再重新推送一次
			$is_send = M('item_order')->where(array('orderId'=>$order['orderId']))->getField('is_sendprice');
			if($is_send == 0){
				foreach($royArr as $rk=>$rv){
					$all_fclist = M('user_allfcllist');
					$fcdata['price']= round($orderPrice,2); //订单总金额
			...
			//结算成功，跳转至支付完成页面
			if(M('item_order')->where(array('orderId'=>$order['orderId']))->getField('is_sendprice') == 1){
...
/*         $users=$user->where($where)->select();
    ...
    */
    ...
    public function yongjin(){
            ...
            //待结算的金额
             $yet = M('user_allfcllist')->where(array('uid'=>$this->visitor->info['id'],'is_find'=>1))->sum('fencheng');
             $yet = $yet ? $yet : 0;
            //昨日佣金
            $data['uid']=$uid;
            $data['state']=1;
    ...
    //佣金详情
     public function yongjin_detail(){
        $time=$_GET['time'];
        $state = $_GET['state'];
        $is_find = $_GET['is_find'];
        ...
        if(!empty($is_find)&&$is_find==1){
           $state = 1;
           $data['is_find'] = $is_find;
           $user_fengchengllist = M('user_allfcllist');
        }
        $data['state'] = 1;
        if ($time=='one') {
     ...
     */
      ...
      public function order_list(){
		//如果是消费者进入，推送一个默认店铺
		$public_shop = $this->_get('public_shop','trim');
		if(!empty($public_shop) && $public_shop == 1){
			if($this->visitor->info['uid'] == 4){
				$_SESSION['shopid'] = 76;
			}
		}
