...
class itemAction extends backendAction {
	...
	public function aprice(){
		$itemMod = M('item');
		$map['id'] = array('in','696,697,1137,1138,2025,2135,2136,2146,2835,4668,2718,3168,3157,3158,3159,3161,3163,3164,3165,3166,3169,3170,3171,3786,3788,3790,3792,3794,3796,3798,4363,4456,4459,4667,4776,5133,5214,5250,4042,4043,2293,2294,2295,2296,2297,2298,2299,2300,2301,2302,2303,2304,2305,2306,2308,2309,2310,2311,2359,4044,4084,4167,4992,4993,4996,4998,4999,5000,5001,5002,5004,5006,5008,5010,5012,3116,2397,3114,2870,2871,3115,3118,4399,4400,4401,4793,4795,4797,5222,420,423,424,425,426,3840,540,3842,3844,4112,2188,2184,2186,2189,2190,2479,2480,2482,2504,2717,2780,2781,2782,2783,2790,2791,2792,2800,2801,2802,2803,2804,3782,3827,3829,3831,3833,3835,3839,3879,3880,3881,3882,3883,3884,3885,3886,3887,3888,3889,3890,3891,3892,3893,3894,4111,4113,4114,4164,4165,4166,4169,4264,4265,4266,4267,4268,4851,4852,4933,4937,4938,4939,5040,5042,5043,5210,4498,4499,4500,4501,4502,4503,4504,4505,4506,486,487,489,504,505,506,507,509,516,517,1691,1692,1866,2367,2369,2378,4029,4028,4030,4031,4032,4033,4034,4035,4036,4037,4038,4040,4041,4604,4607,4608,4611,374,377,874,875,3298,3303,2169,2170,3283,3288,3293,2879,3278,4163,4250,4688,4689,4690,4691,370,371,372,373,3485,3491,3497,3503,3510,3378,2875,2876,2877,2878,3384,3390,3396,3402,4994,2760,5266,1878,1879,1880,1881,1882,1883,1884,1885,1887,1886,1889,1891,4396,4397');
		$items = $itemMod->where($map)->field('id,yhprice,cost,size')->order('brandId asc')->select();
		

		foreach($items as $key=>$val){

			
			$priceArr = explode('|',$val['yhprice']);
			$costArr = explode('|',$val['cost']);
			$apriceArr = '';
			foreach($priceArr as $k=>$v){
				$apriceArr[] = round(($costArr[$k]+$priceArr[$k])/2).'.00';
			}
			$aprice = implode('|',$apriceArr);
			
			
			$itemMod->where(array('id'=>$val['id']))->save(array('aprice'=>$aprice/* ,'activity'=>1 */));
		}
		echo json_encode(array('status'=>1,'msg'=>'success'));

    /*  $data=$this->_mod
			  ->field('id,title,cost,price,yhprice,aprice,status,goods_stock,itemtype,price_jinhuo,brandId,activity')
			  ->where($map)
			  ->select();
		
		 $filename="品牌活动商品信息";
        $headArr=array("ID","商品名称","成本价","销售价","优惠价","活动价","是否上架(0为下架,1为上架)","库存数量","商品类型","进货价","商品所属品牌,活动状态");
...
public function add() {
        ...
        if (IS_POST) {
			...
			$goods_stock = $this->_post('goods_stock');
			$stock = $this->_post('stock');
			$adress = $this->_post('adress');
			$price_jinhuo = $this->_post('price_jinhuo');
			$yhprice = $this->_post('yhprice');
			$aprice = $this->_post('aprice');
