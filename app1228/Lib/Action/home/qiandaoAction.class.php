<?php
class qiandaoAction extends userbaseAction
{
	public $u_phone 	= '';
	public $u_name 		= '';
	...
	{
		//读取抽奖配置
		$qiandao = M('qiandao_config');
		$zp_info = $qiandao->where("id=1")->find();
    ...
    private function getResult($priearr){
		...
		if(($zp_con_info['hd_cishu'] - $you_num) > 0)//如果有抽奖次数
		{
			...
			//添加到积分表中
			$zplistjifen = M('qiandao_list');
			$zp_list_infojifen	= $zplistjifen->where("id=$listid")->find();
			
			$userjifen	= M('item_jifen');
			$jifennum = $zp_list_infojifen['praisecontent'];
			
			$jifen['uid']		= $user_id;
			$jifen['title']	= '积分签到';
			$jifen['jifen']		= $jifennum;
			$jifen['add_time']	= time();
			$jifen['state']		= 1;
			$userjifen->data($jifen)->add();
			...
			if($listid != 7)//如果中奖
			{
				$msg = "恭喜你获得【".$zp_list_info['praisecontent'].'积分】';
			}else{
				$result['angle'] = $result['angle']+mt_rand(1,6)*60;
				$msg	= "请您明天再接再厉，本次签到没有中奖哦!!";
			}
			$result['msg']		= $msg;
		}else{
			$num=0;
    ...
    private function getRand($proArr,$proCount){
		...
		foreach($proCount as $key=>$val){
    		if($val==0){
    			...
    			if($proArr[$key] < $sailMinTmp) $sailMinTmp = $proArr[$key];
    		}
    	}
		$sail10Tmp = 0;
		while(true) {
			$sail10Tmp++;
			$sailMinTmp = $sailMinTmp * 10;
			if($sailMinTmp == intval($sailMinTmp)) break;
		}
		$sail10Tmp = pow(10,$sail10Tmp);
    	...
    	foreach($proCount as $key=>$val){
    		if($val==0){
    			...
    			$proSum=$proSum+$proArr[$key]*$sail10Tmp;
				//echo $proArr[$key]*$sail10Tmp."</br>";
    		}
    	}
		//echo $proSum;
		//exit();
    	...
    	foreach ($proArr as $key => $proCur) {
    		if($proCount[$key]==0){
    			...
    			$randNum = mt_rand(1, $sail10Tmp);//关键
				//echo $randNum."</br>";
				//echo $proCur*$sail10Tmp."</br>";
        		if ($randNum <= $proCur*$sail10Tmp) {
        			$result = $key;
           	 		break;
        		}else{
            		//$proSum -= $proCur*$sail10Tmp;
        		}
    		}
