...
$(function(){
    ...
    //默认载入左侧菜单
    $('#J_lmenu').load($('#J_lmenu').attr('data-uri'),function(){
			$("#J_lmenu ul").first().show();
		});
    ...
    $(".J_switchs").live('click', function(i){
		...
		ul.siblings("ul").hide();
		$(this).parent().siblings("h3").find(".J_switchs").removeClass("on");
});
$("#J_lmenu .f14").live('click', function (i) {
    var ul = $(this).next("ul");
    if (ul.is(':visible')) {
        ul.hide();
    ...
    ul.siblings("ul").hide();
    $(this).siblings("h3").find(".J_switchs").removeClass("on");
});
