...
/*=======================by lyye 2014-04-08=======================*/
   ...
   public function excelout(){
		$params = urldecode($this->_post('map','trim'));
		...
		($time_start = $arr['time_start']) && $map['add_time'][] = array('egt', strtotime($time_start));
        ($time_end = $arr['time_end']) && $map['add_time'][] = array('elt', strtotime($time_end)+(24*60*60-1));
        ($price_min = $arr['price_min']) && $map['price'][] = array('egt', $price_min);
        ($price_max = $arr['price_max']) && $map['price'][] = array('elt', $price_max);
        ($uname = $arr['uname']) && $map['uname'] = array('like', '%'.$uname.'%');
        $cate_id = $arr['cate_id'];
        if ($cate_id) {
            $id_arr = $this->_cate_mod->get_child_ids($cate_id, true);
            $map['cate_id'] = array('IN', $id_arr);
        ...
        $status>=0 && $map['a.status'] = array('eq',$status);
        
        if( $arr['itemtype']==null ){
            $itemtype = -1;
        }else{
            $itemtype = intval($arr['itemtype']);
        }

        if( $arr['home']==null ){
            $home = -1;
        }else{
            $home = intval($arr['home']);
        }
		if( $arr['is_combo']==null ){
            $is_combo = -1;
        }else{
            $is_combo = intval($arr['is_combo']);
        }
		if( $arr['activity']==null ){
            $activity = -1;
        }else{
            $activity = intval($arr['activity']);
        }
		if( $arr['zhuangui']==null ){
            $zhuangui = -1;
        }else{
            $zhuangui = intval($arr['zhuangui']);
        }
		if( $arr['zhuanti']==null ){
            $zhuanti = -1;
        }else{
            $zhuanti = intval($arr['zhuanti']);
        }
		if( $arr['is_delete']==null ){
            $is_delete = 0;
        }else{
            $is_delete = intval($arr['is_delete']);
        }
		
		if( $arr['itembase']==null ){
            $item_base =-1;
        }else{
            $item_base = intval($arr['itembase']);
        }
		
		if( $arr['is_bargain']==null ){
            $is_bargain =-1;
        }else{
            $is_bargain = intval($arr['is_bargain']);
        }
		
        if ($arr['goods_stock'])
        {
            $map['goods_stock']=array('lt',$arr['goods_stock']);
        }
        $itemtype>=0 && $map['itemtype'] = array('eq',$itemtype);
        $home>=0 && $map['home'] = array('eq',$home);
        $is_combo>=0 && $map['is_combo'] = array('eq',$is_combo);
		...
		$item_base>=0 && $map['baseid'] = array('eq',$item_base);
		$is_bargain>=0 && $map['is_bargain'] = array('eq',$is_bargain);
        ($keyword = $arr['keyword']) && $map['title'] = array('like', '%'.$keyword.'%');
		//添加sku编码关键字搜索
        ($sku_nember = $arr['sku_nember']) && $map['adress'] = array('like', '%'.$sku_nember.'%');
        ...
        $this->getExcel($filename,$headArr,$data,6);	
    }
