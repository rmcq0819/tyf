...
class indexAction extends backendAction {
    ...
    public function panel() {
        ...
        $orders = $this->item_order->where($data)->field('orderId,order_sumPrice')->select();
		
		foreach($orders as $key=>$val){
			$idx = substr($val['orderId'],0,18);
			$orderArr[$idx] = $val['order_sumPrice'];
		}
    ...
    public function order_sum_price($date1,$date2,$status){
		...
		$orders=M('item_order')->where($data)->field('orderId,order_sumPrice')->select();
		foreach($orders as $key=>$val){
			$idx = substr($val['orderId'],0,18);
			$orderArr[$idx] = $val['order_sumPrice'];
		}
