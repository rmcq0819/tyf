...
class item_orderAction extends backendAction {
    public function _initialize() {
        ...
        $order_status=array(1=>'待付款',2=>'待发货',3=>'待收货',4=>'已完成',6=>'退货中',7=>'退款失败',8=>'退款成功',9=>'待收货 (清关中)',10=>'异常订单',11=>'退款中');
    ...
    {	
        ...
        foreach ($list as $key => $value) {
			...
			if(M('order_detail')->where(array('orderId'=>$value['orderId'],'status'=>array('in','6,9,10,11')))->find()){
				 $list[$key]['op'] = 1;
			}
        }
...
//2016-04-21 by shuguang 修改列表显示 end
    ...
    public function _before_index() {
		$cwhere['add_time'] = array('lt',time()-24*3600*15);
		$cwhere['status']	= 3;
	  	$orders= M('item_order')->where($cwhere)->select();
		...
		foreach ($orders as $k1 => $v1){
            ...
            foreach($order_detail as $v2){
                $direct = M('item')->where(array('id'=>$v2['itemId']))->field('is_direct,add_time')->find();
		...
		$this->assign('confirm_count',count($orders));
    }
    ...
    public function status()
    {
	  ...
	  if($status == 8){
		...
		$odtkMd = M("item_ordertk");
		$odtkMd->where(array('id'=>$tkId))->save(array('status'=>0));
		
	 }
    }
    public function change_odstatus(){
		$tkId = $this->_get('tkId','intval');
		$detailId = $this->_get('detailId','intval');
		$status = $this->_get('status','intval');
		
		$odMd = M("order_detail");
		$odtkMd = M("item_ordertk");
		$odMd->where(array('id'=>$detailId))->save(array('status'=>$status));
		$odtkMd->where(array('id'=>$tkId))->save(array('status'=>0));
    ...
    protected function _search() {
		...
		$map['weixin_item_order.is_delete'] = array('eq','0');
	...
	public function delete_order(){
		...
		$order = $this->_mod->where(array('id'=>$id))->field('orderId,status')->find();
	
		if($order['status']==1){
			if (false !== $this->_mod->delete($id)) {//删除订单成功
				//删除订单条目
				M('order_detail')->where(array('orderId'=>$order['orderId'].''))->delete();
                if ($_SESSION['content']) {
			...
			$this->_mod->where(array('id'=>$id))->save(array('is_delete'=>1));
			IS_AJAX && $this->ajaxReturn(1, L('operation_success'));
            $this->success(L('operation_success'));
		}
	}
    ...
    public function edit() {
        ...
        if (IS_POST) {
			...
			$finish = M('order_detail')->where(array('orderId'=>$item['orderId']))->field('status')->select();
            if(count($finish)>1){
            ...
            if($not_go == 3 && $laerdy == 4){
                $finish = 1;
            }else{
                $finish = 0;
            }
            $res = M('item_order')->where(array('id'=>$id))->setField('is_finish',$finish);

			//统计订单详情对应的order_detail表中有多少待发货商品
			$contwhere['status'] = array('in','0,2');
            ...
            $order_detail=M('order_detail')->join('a left join weixin_itembase b ON a.baseid=b.id')
			...
			$itemtk = M('item_ordertk')->where(array('orderId'=>$item['orderId']))->distinct('detailId')->select();
			if(count($itemtk)>0){
				foreach($itemtk as $key=>$val){
					$itemtk[$key]['bank'] = M('bank')->where(array('id'=>$val['kaihuhang']))->getField('bank');
				}
...
*/
		...
		//分单处理
		$orderList = M('item_order')->field('a.*,b.itemId,b.title,b.img,b.price,b.quantity,b.size,b.itemtype,b.idcname,b.idcnum,b.idczimg,b.idcfimg')
	...
	public function free_info(){
		...
		}elseif($order['userfree'] == '速腾快递'){
			$result  = $express -> getorder($order['freecode'],3);
		}else{
	...
	public function fahuo(){
        ...
        if (IS_POST&&$this->_post('orderId','trim')) {
			...
			if($_POST['itemlist'] == 0){
				...
				M('order_detail')->where(array('orderId'=>$orderId,'userfree'=>0,'freecode'=>0,'status'=>array('lt',3)))->save($idata);
            ...
            if($save_free || $save_orderfree){
				...
				if($_POST['itemlist'] == 0){
					...
					$content = "亲爱的饭团{$orderlist['username']}您好！您购买的宝贝：{$order['title']}|{$order['size']}，数量：{$order["quantity"]}件，已经乘坐{$order["userfree"]}({$order["freecode"]})迫不及待的奔向您啦，请注意签收 :) \n\n提示：不同商品可能涉及到不同的仓库，因发货仓库不同，货物配送到您手中的时效会稍有区别，请以实际为准。祝您生活愉快！";
				}
	...
	/*	$map = session('map');
		...
		$orders = M('item_order')->join('a left join weixin_order_detail b on a.orderId=b.orderId left join weixin_user c on a.userId=c.id')
							     ->where(array('a.add_time'=>array('gt','1483891200'),
											 'a.add_time'=>array('lt','1483977600'),
											 'a.status'=>array('gt','1'),
											 'b.itemId'=>array('eq',5409)
											 ))
								 ->field('a.orderId as orderId,a.status,c.id as id,c.uid,c.username,a.shopId')
							     ->select();
		foreach($orders as $k=>$v){
			...
			$orders[$k]['shop'] = M('user')->where(array('id'=>$v['shopId']))->getField('username');
			$orders[$k]['merchant'] = M('user')->where(array('id'=>$v['shopId']))->getField('merchant');
		}			
		$filename="台湾老姜 黑糖 280g 两罐";
		$headArr=array("订单号","订单状态","用户ID","用户等级","用户昵称","店铺ID","店家昵称","店铺名称");
		 $this->getExcel($filename,$headArr,$orders,12,0);
    }
