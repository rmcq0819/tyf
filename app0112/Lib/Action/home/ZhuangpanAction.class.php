<?php
class ZhuangpanAction extends userbaseAction
{
	...
	{	
		...
		if($cost == 'free'){
			...
			$start = mktime(0,0,0,date("m",$t),date("d",$t),date("Y",$t));
			$end = mktime(23,59,59,date("m",$t),date("d",$t),date("Y",$t)); 
			$count = M('user_lottery')->where('draw_time between '.$start.' and '.$end.' and userId = '.$this->visitor->info['id'])->count();
			if($count>0){
				echo json_encode('今天的免费抽奖次数已用完啦(>﹏<。)～用10张范票再抽一次？！');
				exit;
			}
		}else if($cost == 'pay'){
			$user_points = M('user')->where(array('id'=>$this->visitor->info['id']))->getField('points');
			if($user_points>=10){
				$message = M('messagelist');
				$message->user_id = $this->visitor->info['id'];
				$message->recom = $this->visitor->info['id'];
				$message->type = 21; //换取大转盘抽奖机会
				$message->time = time();
				$message->status = 0; //默认未读状态
				$message->points = 10;
				$message->add();
				M('user')->where(array('id'=>$this->visitor->info['id']))->setDec('points',10);
				$count = 0;
			}else{
				echo json_encode('您的范票不够啦，快快想办法赚取些再来吧？！');
				exit;
			}				
		}*/
		$chance = M('user')->where(array('id'=>$this->visitor->info['id']))->getField('chance');
		if(!$chance){
			echo json_encode('您暂时没有抽奖机会，参与朋友转发的公益投票链接可获得抽奖机会！');
			exit;
		}
		$ret = $this->getResult();
		//减少用户抽奖机会
...
/* 	public function lottery(){
		$shares = $this->_get('userId','intval');
		$lottery = M('user_lottery')->join('a left join weixin_user b on a.userId=b.id left join weixin_zhuanpan_list c on a.prizeId=c.id')->where(array('userId'=>$shares))->field('a.*,b.username,b.cover,c.pic,c.prize')->order('a.id desc')->find();
		if($lottery){
			$this->assign('lottery',$lottery);
			$this->display();
		}else{
			$this->redirect('Zhuangpan/index');
		}
	} */
    ...
    public function getResult()
	{
		...
		/* for($i=0;$i<2000;$i++){
			$rid['number'] = $this->get_rand($arr); //测试
			M('tmp')->add($rid);
		}
		exit; */
