...
class ShopcartAction extends frontendAction {
    ...
    public function index(){
		...
		foreach ($icarts as $v){
			...
			if(time()>$satime&&time()<$eatime&&$shop['activity']==1){
				//判断是否存在规格
				if(!empty($v['size'])){
					$price = explode('|',$shop['aprice']);
					...
					if($shop['aprice']!=$v['price']){
						$cedit_data['price'] = $shop['aprice'];
	    ...
	    foreach($item as $k=>$v){
			...
			if(count($itemsbuy) < 30){
				$itemsbuy = M('item')->where('status=1')->order('paynum desc')->limit(30)->select();
			}
...
*/
    ...
    public function change_quantity()
    {
		...
		*/
    	...
    	if($stock<$quantity)
    	{
			$sql_up="update weixin_cart set num='".$stock."' where mainid=".$itemId;
			$data_up=mysql_query($sql_up);
			$cart->modNum($seid,$stock);
			$cartItem = M('cart')->where(array('mainid'=>$itemId))->field('num,price')->find();
			$data=array('status'=>0,'item'=>$cartItem,'stock'=>$stock,'msg'=>'该商品的库存不足');
    	}else {	
			...
			$cartItem = M('cart')->where(array('mainid'=>$itemId))->field('num,price')->find();
			$data=array('status'=>1,'item'=>$cartItem,'sumPrice'=>$cart->getPrice(),'discount'=>$cart->getdiscount()); 
    	}
