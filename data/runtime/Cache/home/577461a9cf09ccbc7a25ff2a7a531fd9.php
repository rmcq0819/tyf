...
.login2{ color:#009900; text-align:center; display:none;}
		...
		<style type="text/css">
			*{
			...
			#main{
				position: relative; left: 0px; margin-bottom: -5px;
			}
			#main .center{
				width: 280px;position: absolute; left: 50%; top: 45%; margin-left:-140px ;
			}
			#main .center .content{
				width: 70%;text-align: center; margin: 0 auto; margin-top: 5px; color: rgb(240,93,0); overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
			}
			#main .center .content2{
				width: 280px;text-align: center; margin: 0 auto; margin-top: 5px; color: rgb(180,47,33); font-size: 22px; margin-top: 12px;
			}
			#main .center input{
				width: 180px;height: 27px;border: solid 1px rgb(180,47,33); padding-left: 5px; color: rgb(85,85,85); margin-top: 3px;
			}
			#main .center button{
				width: 220px; height: 35px; background-color: rgb(180,47,33); border: none; color: rgb(252,243,2); border-radius: 5px; font-size: 16px; 
			}
			/*验证消息弹出框*/
			.showinfo {
	...
	<body>
		...
		<div id="main">
			<img src="../Style/index-images/activity/p-bg5.jpg" alt="头部图片" class="am-img-responsive" style="margin-top: 49px;"/>
			<div class="center">
				<p style="text-align: center;"><img src="../Style/index-images/activity/p-user.png" alt="头像" class="am-circle" style="width: 70px;"/></p>
				<p class="content">你的好友</p>
				<p class="content2" style="line-height: 32px;">在礼包内给你留的新年祝福</p>
				<div style="width: 290px;margin: 0 auto; margin-top: 15px; color: rgb(180,47,33);">
					<p>输入收货手机号码，看好友是否为你留下了专属祝福</p>
					<p style="text-align: center;"><input type="number" placeholder="输入手机号码"/></p>
				</div>
				<p style="text-align: center; margin-top: 25px;"><button id="submit">领取祝福</button></p>
			</div>
			
			
			<img src="../Style/index-images/activity/p-bg6.jpg" alt="底部图片" class="am-img-responsive" />
		...
		<script type="text/javascript">
			$(function(){
				$("#submit").on("click",function(){
					var phone_m = $("input[type=number]").val();
					if(phone_m==''){
						var content = $('.showinfo').html('请输入手机号码');
					...
					if(!phone.test(phone_m)){
						var content = $('.showinfo').html('手机号码无效');
						var w = content.width()/2;
						$(".showinfo").css("margin-left",-w);
						$(".showinfo").show().delay(3000).fadeOut();
						return false;
					}
					location.href="<?php echo U('Blessing/blessing');?>&phone="+phone_m;
				});
			})
		</script>
	</body>
