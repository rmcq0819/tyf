...
.login2{ color:#009900; text-align:center; display:none;}
	...
	<body>
		...
		<form method="POST" action="<?php echo U('Order/pay');?>&spr=<?php echo $_GET['spr']; ?>" id="order_form" name="order_form" enctype="multipart/form-data">
			...
			<div style="padding: 10px; margin-bottom: 150px;">
				<p style="color: rgb(240,93,0); font-size: 14px;">温馨提示</p>
				<p style="color: rgb(240,93,0); text-align: left;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;亲爱的范团，感谢您对我们的支持和信任，
					即日起，购买新年礼包可以添加自己想对家人或者亲人朋友说的祝福语，
					还可以上传图片或者视频来表达您的祝福，我们会将祝福和礼包一起送到，
					祝您在商城购物愉快，新年快乐！<span style="color: rgb(85,85,85);">（点击“添加祝福”图标进入祝福通道）</span>
				</p>
			</div>
			<div>
				<img src="../Style/index-images/activity/up1.png" alt="添加祝福" style="width: 80px; position: fixed; right: 3px; bottom: 120px;"/>
			</div>
			...
			<div id="footer">
				<div class="am-g">
  				...
  				<span id="yunfei">运费:</span>
						...
						<?php if($is_fictitious["is_fictitious"] == 1 OR $store["is_store"] == 1): ?>￥0
							<?php else: ?>
  				...
  				<span style="font-size: 13px;">合计:</span>
						...
						<?php if($is_fictitious["is_fictitious"] == 1 OR $store["is_store"] == 1): ?>￥<?php echo ($sunprice); ?>
							<?php else: ?>
			...
			<span ><?php echo ($vo2["sheng"]); echo ($vo2["shi"]); echo ($vo2["qu"]); echo ($vo2["address"]); ?></span>
					...
					<p style="padding:10px;" id="add_addr">
						<a href="<?php echo U('User/addaddress',array('ids'=>$_GET['ids']));?>" style="float:right;">
