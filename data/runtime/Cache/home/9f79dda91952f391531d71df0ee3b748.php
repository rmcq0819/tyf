...
.login2{ color:#009900; text-align:center; display:none;}
		...
		<style type="text/css">
			*{
			...
			#order{
				background-color: white; border-bottom: solid 1px rgb(237,237,237);
			}
			#comm .num{
				border-bottom: solid 1px rgb(200,200,200); padding: 7px; color: rgb(85,85,85); background-color: white;
			}
			
			#order .pro-name{
				margin-top: 5px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; padding-left: 9px; color: rgb(85,85,85); font-size: 12px;
			}
			#order .pro-pri{
				margin-top: 10px;color: rgb(138,138,138);padding-left: 9px;  font-size: 12px;
			}
			/*去评价按钮*/
			.go-comm{
				background-color: white; border:solid 1px rgb(240,93,0); color: rgb(240,93,0); float: right; margin-right: 5px; border-radius: 3px; outline: none;
			}
		</style>
	</head>
	...
	<body style="background-color: rgb(243,243,243);">
		...
		<div id="comm">
			<ul>
				<p class="num" style="margin-top: 49px;">订单号：<?php echo ($orderId); ?> </p>
				<?php if(is_array($item_detail)): $i = 0; $__LIST__ = $item_detail;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li>
					<div id="order">
						<div class="am-g" style="padding: 7px;">
  							<div class="am-u-sm-3">
  								<img src="data/upload/item/<?php echo ($vo['img']); ?>" alt="商品" class="am-img-responsive" style="width:80px;"/>
  							</div>
  							<div class="am-u-sm-9">
  								<p class="pro-name"><?php echo ($vo["title"]); ?></p>
  								<p class="pro-pri">优品价：<span style="color: rgb(240,93,0); font-size: 13px;">￥<?php echo ($vo["price"]); ?></span></p>
  							</div>
  							<button class="go-comm" onclick="location.href='<?php echo U('User/do_comment',array('orderId'=>$orderId,'itemId'=>$vo['itemId']));?>'">去评价</button>
						</div>
					</div>
