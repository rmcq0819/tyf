		  ...
		  <div class="am-modal-dialog" style="background: none;">
		    <div class="am-modal-bd" style="position: relative;left: 0px;">
		      	<div style="height: 220px;background-color: rgb(229,229,229); border: solid 2px rgb(240,93,0); border-radius: 5px;">
		      		...
		      		<button style="width: 100px; height: 28px;background-color: rgb(241,93,53); color: rgb(240,197,62); border: solid 1px rgb(240,197,62);border-bottom: solid 2px rgb(240,197,62); border-radius: 10px; position: absolute; left: 50%; top: 175px; margin-left: -50px;" onclick="location.href='<?php echo U('User/index');?>'">去查看奖励</button>
		      	</div>
		    </div>
		  </div>
		</div>
		...
		<script type="text/javascript">
			$(function(){
				$("#submit").on("click",function(){
					...
					var url="<?php echo U('Activity/public_do_signup');?>";
					
					$.getJSON(url,{date:date,uname:uname,phone:phone,wechat_num:wechat_num},function(data){
						if(data.status==1){
							$("#btn_success").click();
						}else{
							layer.open({
