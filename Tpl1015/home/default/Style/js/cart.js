...
function change_quantity(rec_id,seid,input){
  ...
  if(isNaN(input.value)||input.value<1) 
  {
   ...
   $.post("index.php?m=Shopcart&a=change_quantity",{itemId:rec_id,seid:seid,quantity:_v},function(data){
 
    	if(data.status==1)
    	{
		//subtotal_span.html(price_format(data.item.price*data.item.num*data.discount));
    	//var price = round(paresFloat(data.item.price),2);
